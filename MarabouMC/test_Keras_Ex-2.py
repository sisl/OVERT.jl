...
x_0_ce = []
for i in range(4):
    x_0_ce.append(value["%s@0"%states[i]])
print(x_0_ce)
from gym_new.pendulum_new import Pendulum2Env
env = Pendulum2Env(dt = dt, x_0=x_0_ce)
env.reset()
for j in range(100):
    T = model.predict(np.array(x_0_ce).reshape(1, 4)).reshape(2,)
    env.step(T)

env.animate(dpi=60, file_name="/home/amaleki/Downloads/pend2.gif")
