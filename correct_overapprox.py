    ...
    for i in range(nsteps):
        ...
        with tf.name_scope("relu_protection"):
            for i in range(ncontroller_act):
                theta = relu_protector.apply(theta)
                theta_dot = relu_protector.apply(theta_dot)
                theta_dot_LBs = [relu_protector.apply(tdlb) for tdlb in theta_dot_LBs]
                theta_dot_UBs = [relu_protector.apply(tdub) for tdub in theta_dot_UBs]
                theta_dot_hats = [relu_protector.apply(tdh) for tdh in theta_dot_hats]
...
feed_dict = {
        ...
        'assign_init_vals/theta_dot_hat_1:0':np.array([[0.0]]),
        'assign_init_vals/theta_dot_hat_2:0':np.array([[0.0]]),
        'assign_init_vals/theta_dot_hat_3:0':np.array([[0.0]]),
    }
sess.run(tf.global_variables_initializer())
theta, theta_dot, theta_dot_hats, theta_dot_LBs, theta_dot_UBs = sess.run([theta, theta_dot, theta_dot_hats, theta_dot_LBs, theta_dot_UBs], feed_dict=feed_dict)
print("theta: ", theta)
print("theta_dot: ", theta_dot)
print("theta_dot_hats: ", theta_dot_hats)
print("theta_dot_LBs: ", theta_dot_LBs)
print("theta_dot_UBs: ", theta_dot_UBs)
