...
function check_concave_upper_optimality(y_i_m, y_i_p; ϵ=1e-6)
	"""
	...
	"""
	#@debug(println("y_i_m=", y_i_m, ", y_i_p=", y_i_p))
	#@debug(println("y_i_m - y_i_p = ", y_i_m - y_i_p))
	@info "Continuity violation threshold is: ,Max abs continuity violation is: " ϵ maximum(abs.(y_i_m - y_i_p))
	return all(abs.(y_i_m - y_i_p) .<= ϵ)
end
