basepath=/home/csidrane/aahaa_files

# export PYTHONPATH=$basepath/Dropbox/AAHAA/src
#export PYTHONPATH=$basepath/Dropbox/AAHAA/src/rllab:$PYTHONPATH
#export PYTHONPATH=$basepath/Dropbox/AAHAA/src/RNNs4Marabou:$PYTHONPATH
export PYTHONPATH=$basepath
export PYTHONPATH=$basepath/OverApprox:$PYTHONPATH
export PYTHONPATH=$basepath/Marabou:$PYTHONPATH
export PYTHONPATH=$basepath/gym:$PYTHONPATH
