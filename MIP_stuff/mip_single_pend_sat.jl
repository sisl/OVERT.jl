...
dt = 0.1
N_OVERT = 10
input_set = Hyperrectangle(low = [1.0, 1.0], high = [2.0, 2.0])
unsafe_set = Hyperrectangle(low = [2.5, 2.3], high = [3.0, 2.7])
input_vars = [:th, :dth]
control_vars = [:T]
...
exact_dynamics = single_pend_dynamics

all_sets, all_counter_examples, violation  = satisfiability_search(dynamics, update_rule, network_file, input_set,
...
#          end
#       end
#       return all_sets, counter_example_list
# end

# plot_output_sets(all_sets; idx=[1,2])
# plot!(rectangle(0.5,0.5,2.5,2.0), fillalpha=0.7, fill=:red, legend=nothing, linewidth=3)
# scatter!(counter_example_list[])
#
# xc = [c[1] for c in counter_example_list]
# yc = [c[2] for c in counter_example_list]
# push!(xc, 2.5)
# push!(yc, 2.13)
# plot!(xc, yc, marker=:o, markersize=5, linewidth=2)
#
# xd = [1.677, 1.875, 2.07, 2.286, 2.498]
# yd = [1.977, 2.032, 2.08, 2.120, 2.142]
#
# plot!(xd, yd, marker=:o, markersize=5, linewidth=2)
...
# plot!(rectangle(w,h,x,y), fillalpha=0.7, fill=:red, legend=nothing,
#           linewidth=linewidth, linecolor=linecolor, linestyle=linestyle)
#
# xlabel!("x_$(idx[1])")
#
