...
start_range = ([-10*np.pi/180, -120*np.pi/180],
			   [10*np.pi/180, 120*np.pi/180])
#([-10*np.pi/180, -120*np.pi/180],[10*np.pi/180, 120*np.pi/180])

safe_range = ([-10*np.pi/180, -120*np.pi/180],[10*np.pi/180, 120*np.pi/180])
...
with tf.Session() as sess:
	sim_system(file, policy_name, start_range, safe_range, nsteps, nsims, wrap_fn, labels=labels, plot_trajs=True, rand=False)
